...
- `dts/cq8725s-ayn-common.dtsi.patch`
  source: armada
  notes: Armada keeps volume-up from waking the system, marks Odin 3's RSInput node as connected to the Qualcomm haptics device, and supplies the device's 1024 range. The 70-count `axis-deadzone` was dropped with kernel patch 1301, which centres the sticks; like every other RSInput device the Odin 3 now passes the centred value through.
